...
// PRE: El string recibido debe ser válido.
// POST: Devuelve un número positivo en base al string recibido.
unsigned long funcion_hash(unsigned char *str);

// PRE: El string recibido debe ser válido.
// POST: Devuelve una copia del string recibido por parámetro reservada en el
// heap.
char *strdup(const char *str);

// PRE: La clave recibida debe ser válida.
// POST: Devuelve, en caso de no haber error, un nuevo par clave/valor.
par_t *crear_par(const char *clave, void *elemento);

// PRE: El hash, el nuevo par y el indice recibidos deben ser válidos.
// POST: Inserta el par clave/valor en la tabla hash.
hash_t *hash_insertar_par(hash_t *hash, par_t *nuevo_par, size_t indice);

// PRE: El hash recibido debe ser válido.
// POST: Devuelve, en caso de no haber errores, un nuevo hash con mas capacidad
// y los pares previos.
  ...
  for (size_t i = 0; i < hash->capacidad; i++) {
    if (hash->tabla[i] != NULL) {
      ...
      while (par_actual != NULL) {
        par_t *par_auxiliar = par_actual;
        par_actual = par_actual->siguiente;
        size_t indice = (funcion_hash((unsigned char *)par_auxiliar->clave)) %
                        nuevo_hash->capacidad;
        hash_insertar_par(nuevo_hash, par_auxiliar, indice);
      }
    }
  }
